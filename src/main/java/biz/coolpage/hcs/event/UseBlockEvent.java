...
public class UseBlockEvent {
    ...
    public static void init() {
        UseBlockCallback.EVENT.register((player, world, hand, hitResult) -> {
            if (player != null && !player.isSpectator() && player instanceof ServerPlayerEntity serverPlayer) {
                ...
                if ((block == Blocks.DIRT || block == Blocks.GRASS_BLOCK || state.isReplaceable() /* material == Material.REPLACEABLE_PLANT */) && (stateUp.isReplaceable()|| blockUp == Blocks.AIR || blockUp == Blocks.CAVE_AIR) && mainHand == Reg.BERRY_BUSH) {
                    if (!player.isCreative()) mainHandStack.decrement(1);
                    world.setBlockState(state.isReplaceable() ? (world.getBlockState(pos.down(1)).isReplaceable() ? pos.down(1) : pos) : posUp, Blocks.SWEET_BERRY_BUSH.getDefaultState());
                }
    ...
    public static void onDrinkWaterWithBareHand(ServerPlayerEntity player, BlockPos pos) {
        ...
        if (player.isSneaking() && player.getMainHandStack().isEmpty() && player.getOffHandStack().isEmpty()) {
            ...
            if (WorldHelper.IS_SALTY_WATER_BIOME.test(player.getWorld().getBiome(pos))) {
